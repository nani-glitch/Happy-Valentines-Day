$(document).ready(function() {
    ...
    // Append navigation areas with Valentine-themed, Indian-style descriptive alt texts
    $('.navigation').append('<area shape="poly" coords="224,82, 339,149, 339,279, 224,346, 109,279, 109,149" href="#close" alt="Ishq Ka Jashn – Close Navigation" class="logo">');
    $('.navigation').append('<area shape="poly" coords="147,-5, 219,40, 219,120, 147,160, 74,120, 74,40" href="#!/page_Home" alt="Dil Se – Home: A Declaration of Love" class="nav1">');
    $('.navigation').append('<area shape="poly" coords="73,129, 145,174, 145,254, 73,294, 0,254, 0,174" href="#!/page_About" alt="Our Love Journey – Pyaar Ki Kahani" class="nav2">');
    $('.navigation').append('<area shape="poly" coords="147,261, 219,307, 219,387, 147,427, 74,387, 74,307" href="#!/page_Talk" alt="Romantic Whispers – Dil Ki Baaten" class="nav3">');
    $('.navigation').append('<area shape="poly" coords="300,261, 372,307, 372,387, 300,427, 228,387, 228,307" href="#!/page_Message" alt="Blessings &amp; Wishes – Shubhkamnayein" class="nav4">');
    $('.navigation').append('<area shape="poly" coords="379,129, 451,174, 451,254, 379,294, 306,254, 306,174" href="#!/page_Blog" alt="Cherished Memories – Yaadein" class="nav5">');
    $('.navigation').append('<area shape="poly" coords="300,-5, 372,40, 372,120, 300,160, 228,120, 228,40" href="#!/page_Time" alt="Timeless Moments – Ishq Ka Safar" class="nav6">');

    $('.map_back').append('<area shape="poly" coords="73,-5, 145,40, 145,120, 73,160, 0,120, 0,40" href="#close" alt="Return to Previous Page" id="back">');

    $('.map_logo').append('<area shape="poly" coords="115,-7, 230,60, 230,190, 115,257, 0,190, 0,60" href="#close" alt="Ishq Ka Jashn – Close Navigation" class="logo">');
    ...
    $('.navigation area').hover(function(){
        ...
        if (n === 'logo') {
            ...
            $('#nav' + n + '> img').css({zIndex:8}).stop().animate({width:165, height:177, left:5, top:5}, 600, 'easeOutBack', function(){
                $(this).css({zIndex:7});
            });
        ...
        if (n === 'logo') {
            ...
            n = parseInt(n.slice(-1));
            $('#nav' + n + '> img').stop().animate({width:145, height:157, left:15, top:15}, 600, 'easeOutBack', function(){
                $(this).css({zIndex:6});
            });
    ...
    /* 
    // Uncomment and customize if using fancybox for galleries
    ...
    */
    ...
    // Blog slideshow using cycle plugin
    $('.blog').cycle({
        fx: 'scrollVert',
        pager: '.pagin',  // Selector for pager container
        activePagerClass: 'active', // Class for active pager link
        timeout: false, // Disable auto-advance
        speed: 800, // Transition speed
        prev: '.prev',  // Selector for previous slide trigger
        next: '.next'   // Selector for next slide trigger
    });
    ...
    if ($.browser.msie && $.browser.version == 8) {
        ...
        $('.pagin a').click(function(){
            $(this).stop().css({background:'none', fontWeight:'bold', color:'#1b1b1b'})
                .siblings().css({fontWeight:'normal'})
                .stop().animate({color:'#1b1b1b', backgroundColor:'#ececec'});
        });
        ...
        $('.prev, .next').click(function(){
            $('.pagin .active').stop().css({background:'none', fontWeight:'bold', color:'#1b1b1b'})
                .siblings().css({fontWeight:'normal'})
                .stop().animate({color:'#1b1b1b', backgroundColor:'#ececec'});
        });
    }
...
$(window).load(function() {
        ...
        actFu: function(_) {
            if (_.prev && _.curr) {
                ...
                _.prev.find('.pad').stop().animate({opacity:0}, function(){
                    ...
                    _.prev.find('.back').stop().animate({opacity:0}, function(){
                        $(this).css({top:212, left:163, opacity:1, display:'none'});
                        $(this).css({opacity:'none'});
                    });
                    _.prev.find('.bg_cont').stop().animate({top:303, left:281, width:0, height:0}, 600, 'easeInBack', function(){
                        _.curr.find('.back').css({top:384, left:477, opacity:0, display:'block'})
                        ...
                        _.curr.find('.bg_cont').stop().animate({top:0, left:0, width:562, height:606}, 600, 'easeOutBack', function(){
                            _.curr.find('.pad').css({display:'block'})
                                .stop().animate({opacity:1}, function(){
                                    $(this).css({opacity:'none'});
                                    fl_cont = true;
                                });
    ...
    function googleMap(){
        /*
         // Uncomment and customize this block if you wish to load a Google Map
         if(googleMap.ready)
             return false;
